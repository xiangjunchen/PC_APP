...
// class AFX_EXT_CLASS CTPDBRecordset
// {
// public:
// 	CTPDBRecordset(void);
// 	~CTPDBRecordset(void);
...
public:
	CTPDBRecordset(void);
	virtual ~CTPDBRecordset(void);
private:
	CTPDBRecordset(const CTPDBRecordset &);
	CTPDBRecordset & operator = (const CTPDBRecordset &);
public:
	...
	// 将Recordset转化为xml
	BOOL Recordset2XML(CString & strXML);
	// 从xml中获取Recordset
	BOOL XML2Recordset(const CString & strXML);
