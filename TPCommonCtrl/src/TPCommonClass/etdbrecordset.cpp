...
#include ".\etcommonclassapi.h"

ET_COMMONCLASS_BEGIN

#include <stdio.h>
//#define ETWriteLog(str) \
//{ FILE *fp; fp = fopen("c:\\TestIngest.txt", "a"); \
//	fwrite((LPCTSTR)str, 1, (_tcslen(str) + 1)* 2, fp); fclose(fp);}
...
BOOL CETDBRecordset::Recordset2XML(CString & strXML)
{
	try
	{
...
BOOL CETDBRecordset::XML2Recordset(const CString & strXML)
{
	try
	{
		_StreamPtr pStream;
