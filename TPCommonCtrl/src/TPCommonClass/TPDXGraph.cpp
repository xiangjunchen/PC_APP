...
bool CTPDXGraph::QueryInterfaces(void)
{
	if (mGraph)
	{
		HRESULT hr = NOERROR;
...
bool CTPDXGraph::Attach(IGraphBuilder * inGraphBuilder)
{
	Release();
...
IGraphBuilder * CTPDXGraph::GetGraph(void)
{
	return mGraph;
}

IMediaEventEx * CTPDXGraph::GetEventHandle(void)
{
	return mEvent;
}

// Connect filter from the upstream output pin to the downstream input pin
bool CTPDXGraph::ConnectFilters(IPin * inOutputPin, IPin * inInputPin, 
...
void CTPDXGraph::DisconnectFilters(IPin * inOutputPin)
{
...
bool CTPDXGraph::SetDisplayWindow(HWND inWindow)
{	
	if (mVideoWindow)
	{
	//	long lVisible;
...
bool CTPDXGraph::ResizeVideoWindow(long inLeft, long inTop, long inWidth, long inHeight)
{
	if (mVideoWindow)
	{
...
bool CTPDXGraph::SetNotifyWindow(HWND inWindow)
{
	if (mEvent)
	{
...
void CTPDXGraph::HandleEvent(WPARAM inWParam, LPARAM inLParam)
{
	if (mEvent)
	{
...
bool CTPDXGraph::IsRunning(void)
{
	if (mGraph && mMediaControl)
	{
...
bool CTPDXGraph::IsStopped(void)
{
	if (mGraph && mMediaControl)
	{
		OAFilterState state = State_Stopped;
...
bool CTPDXGraph::SetFullScreen(BOOL inEnabled)
{
	if (mVideoWindow)
	{
...
bool CTPDXGraph::GetFullScreen(void)
{
	if (mVideoWindow)
	{
...
bool CTPDXGraph::GetCurrentPosition(double * outPosition)
{
	if (mSeeking)
	{
		__int64 position = 0;
...
bool CTPDXGraph::GetStopPosition(double * outPosition)
{
	if (mSeeking)
	{
		__int64 position = 0;
...
bool CTPDXGraph::SetCurrentPosition(double inPosition)
{
	if (mSeeking)
	{
...
bool CTPDXGraph::SetStartStopPosition(double inStart, double inStop)
{
	if (mSeeking)
	{
		__int64 one = 10000000;
...
bool CTPDXGraph::GetDuration(double * outDuration)
{
	if (mSeeking)
	{
		__int64 length = 0;
...
bool CTPDXGraph::SetPlaybackRate(double inRate)
{
	if (mSeeking)
	{
...
bool CTPDXGraph::SetAudioVolume(long inVolume)
{
	if (mBasicAudio)
	{
...
long CTPDXGraph::GetAudioVolume(void)
{
	long volume = 0;
	if (mBasicAudio)
	{
...
bool CTPDXGraph::SetAudioBalance(long inBalance)
{
	if (mBasicAudio)
	{
...
long CTPDXGraph::GetAudioBalance(void)
{
	long balance = 0;
	if (mBasicAudio)
	{
...
bool CTPDXGraph::RenderFile(const char * inFile)
{
	if (mGraph)
	{
...
bool CTPDXGraph::SnapshotBitmap(const char * outFile)
{
	if (mBasicVideo)
	{
		long bitmapSize = 0;
...
void CTPDXGraph::AddToObjectTable(void)
{
	IMoniker * pMoniker = 0;
...
void CTPDXGraph::RemoveFromObjectTable(void)
{
	IRunningObjectTable * objectTable = 0;
    if (SUCCEEDED(GetRunningObjectTable(0, &objectTable))) 
	{
